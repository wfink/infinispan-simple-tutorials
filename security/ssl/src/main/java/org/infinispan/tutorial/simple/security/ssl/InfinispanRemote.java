package org.infinispan.tutorial.simple.security.ssl;
...
public class InfinispanRemote {
   ...
   private static void simpleClient1Auth() {
      System.out.println("  - Try to access the server with simple ssl server keystore and client1 authentication");
   ...
   private static void simpleClient2Auth() {
      System.out.println("  - Try to access the server with simple ssl server keystore");
      // Create a configuration for a locally-running server
      ConfigurationBuilder builder = new ConfigurationBuilder();
      builder.addServer().host("127.0.0.1").port(ConfigurationProperties.DEFAULT_HOTROD_PORT).maxRetries(0)
      .security().ssl().trustStoreFileName("server-truststore.pfx").trustStorePassword("trustSecret".toCharArray())
   ...
   private static void signedClient1Keystore() {
      System.out.println("\n\n  - Try to access the server with a signed ssl server and client1 keystore");
      // Create a configuration for a locally-running server
      ConfigurationBuilder builder = new ConfigurationBuilder();
      builder.addServer().host("127.0.0.1").port(ConfigurationProperties.DEFAULT_HOTROD_PORT).maxRetries(0)
   ...
   private static void signedClient2Keystore() {
      System.out.println("\n\n  - Try to access the server with a signed ssl server and client2 keystore");
      // Create a configuration for a locally-running server
      ConfigurationBuilder builder = new ConfigurationBuilder();
      builder.addServer().host("127.0.0.1").port(ConfigurationProperties.DEFAULT_HOTROD_PORT).maxRetries(0)
      .security().ssl().trustStoreFileName("server_truststore.p12").trustStorePassword("ServerTrustsecret".toCharArray())
   ...
   private static void signedClient1KeystoreAuth() {
      System.out.println("\n\n  - Try to access the server with a signed ssl server and client1 keystore with Authorization");
      // Create a configuration for a locally-running server
      ConfigurationBuilder builder = new ConfigurationBuilder();
      builder.addServer().host("127.0.0.1").port(ConfigurationProperties.DEFAULT_HOTROD_PORT).maxRetries(0)
      .security().ssl().trustStoreFileName("server_truststore.p12").trustStorePassword("ServerTrustsecret".toCharArray())
   ...
   private static void signedClient2KeystoreAuth() {
      System.out.println("\n\n  - Try to access the server with a signed ssl server and client2 keystore with Authorization");
      // Create a configuration for a locally-running server
      ConfigurationBuilder builder = new ConfigurationBuilder();
      builder.addServer().host("127.0.0.1").port(ConfigurationProperties.DEFAULT_HOTROD_PORT).maxRetries(0)
      .security().ssl().trustStoreFileName("server_truststore.p12").trustStorePassword("ServerTrustsecret".toCharArray())
