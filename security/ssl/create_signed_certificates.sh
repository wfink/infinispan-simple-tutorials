# Generate the CA certificate. This file can be used by clients to trust the server
...
# Extract the CA certificate to a file. This will be imported into other key/trust stores
...
# Import the CA certificate into the server keystore (required because the keystore needs the full chain)
...
# Create a server truststore containing the CA
...
# Import the CA certificate into the client keystore (required because the keystore needs the full chain)
